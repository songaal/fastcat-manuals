...
검색횟수추이이며, 선택한 기간의 일자별 검색횟수와 이전 기간의 검색횟수를 함께 그래프로 보여준다. 
...
클릭유입율이며, 해당 기간내 일자별 클릭유입건수와 총 검색건수를 선 그래프로 보여준다.
...
<div class="alert alert-warning">카테고리분류는 그 자체가 카테고리가 선택되어 있는 것이므로, 카테고리 선택이 불가능하다.</div>
